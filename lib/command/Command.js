...
class Command {
    ...
    */
    ...
    async process(args, msg) {
        ...
        let label;
        if (typeof this.subcommandAliases[args[0]] === "string") {
            label = this.subcommandAliases[args[0]]
        } else {
            label = args[0]
        }
        let subcommand;
        if(typeof this.subcommandAliases[args[0]] === "string" && (subcommand = this.subcommands[label]) !== undefined || ((subcommand = this.subcommands[label.toLowerCase()]) !== undefined && subcommand.caseInsensitive)) {
